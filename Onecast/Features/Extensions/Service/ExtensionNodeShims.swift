...
final class ExtensionNodeShims: @unchecked Sendable {
    ...
    private func feed(_ input: Data, to stdin: Pipe) {
        ...
        // A child that exits without reading everything must fail the write, not SIGPIPE Onecast.
    ...
    /// `process.kill`, refusing every target that would signal Onecast along with the child.
