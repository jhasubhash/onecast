...
final class ExtensionRuntime: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.onecast.extensions.js", qos: .userInitiated)
    ...
    private func bootOnQueue(config: ExtensionBootConfig) throws {
        ...
        _ = context.objectForKeyedSubscript("__onecast")?
    ...
    func start(
        ...
        await onQueue { context in
            let compiled = context.objectForKeyedSubscript("__onecast")
    ...
    func popNavigation(session: String) async -> Bool {
        await withCheckedContinuation { continuation in
            queue.async {
                ...
                let result = context.objectForKeyedSubscript("__onecast")?
    ...
    /// Installs `__onecastHost` (the JS→Swift seam) and `__onecastCompile`.
    private func installHost(in context: JSContext) {
        ...
        context.setObject(host, forKeyedSubscript: "__onecastHost" as NSString)
        ...
        context.setObject(compile, forKeyedSubscript: "__onecastCompile" as NSString)
    }
    ...
    private func startTimer(id: String, milliseconds: Double, repeats: Bool) {
        ...
        timer.setEventHandler { [weak self] in
            ...
            _ = self.context?.objectForKeyedSubscript("__onecast")?
