...
enum SystemActionRunner {
    ...
    private static func postKey(keyCode: CGKeyCode, flags: CGEventFlags) throws {
        guard Permissions.ensureAccessibility() else {
            throw SystemActionFailure(
                "Allow Onecast to control your Mac in Accessibility settings, then try again.",
    ...
    private static func dismissNotifications() async throws -> Int {
        ...
        for _ in 0..<100 {
            ...
            guard let action = dismissAction(of: notification) else {
                throw SystemActionFailure(
                    "This version of Notification Center exposes no dismiss control Onecast can use.")
            }
    ...
    private static func toggleBluetooth() async throws -> Bool {
        ...
        throw SystemActionFailure(
            "Bluetooth did not change state. Check Onecast’s Bluetooth permission.",
            settings: .bluetooth)
    }
    ...
    private static func runAppleScript(_ source: String) async throws -> AppleScriptResult {
        try await Task.detached(priority: .userInitiated) {
            ...
            if number == -1743 {
                throw SystemActionFailure(
                    "Allow Onecast to control the requested app in Automation settings, then try again.",
                    settings: .automation)
            }
