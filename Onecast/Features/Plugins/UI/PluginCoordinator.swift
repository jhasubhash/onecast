...
final class PluginCoordinator {
        ...
        Task {
            guard
                await core.confirm(
                    ...
                    message:
                        "Plugins are native code that runs inside Onecast with the same access to "
                        + "this Mac that Onecast itself has. Only enable plugins whose source you "
    ...
    /// Open a plugin straight to a saved deep link — a `onecast://plugin/…` quicklink resolved here.
    ...
    /// The scaffold's "Copy Deep Link" command: put the current view's `onecast://…` link on the
    /// pasteboard.
...
/// Encodes a plugin deep link as a `onecast://plugin/<identifier>?<payload>` URL — stored as an
/// ordinary Quicklink, intercepted at open time and dispatched to the plugin, not the browser.
enum PluginRouteURL {
    private static let scheme = "onecast"
