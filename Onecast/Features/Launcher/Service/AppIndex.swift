...
extension AppEntry {
    ...
    init(_ layout: WindowLayout) {
        self.init(
            ...
            url: URL(string: "onecast://window-layout/" + layout.id.uuidString)!,
    ...
    init(_ size: CustomWindowSize) {
        self.init(
            ...
            url: URL(string: "onecast://window-size/" + size.id.uuidString)!,
    ...
    init(_ action: CustomQuickAction) {
        self.init(
            ...
            url: URL(string: "onecast://quick-action/" + action.id.uuidString)!,
    ...
    init(_ quicklink: Quicklink) {
        self.init(
            ...
            url: URL(string: "onecast://quicklink/" + quicklink.id.uuidString)!,
...
final class AppIndex {
    ...
    private static let systemActionEntries: [AppEntry] = SystemActionCatalog.all
        .map { command in
            AppEntry(
                ...
                url: URL(string: "onecast://system-action/" + command.id.rawValue)!,
    ...
    private static let allWindowCommandEntries: [AppEntry] = WindowCommandCatalog.all
        .map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "onecast://window-command/" + command.id.rawValue)!,
                bundleID: nil, kind: .windowCommand)
        }
        .sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
    ...
    func setCustomCommands(_ commands: [CustomCommand]) {
        let entries = commands.filter(\.isEnabled).map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "onecast://custom-command/" + command.id.uuidString)!,
    ...
    func setAssistants(_ assistants: [Assistant]) {
        let entries = assistants.map { assistant -> AppEntry in
            ...
            return AppEntry(
                ...
                url: URL(string: "onecast://assistant/" + assistant.id.uuidString)!,
    ...
    /// Meetings keep their own card, and Onecast opening Onecast goes nowhere.
    ...
    private static let ownBundlePrefix = "com.onecast."
