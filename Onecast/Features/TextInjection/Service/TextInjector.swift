...
final class TextInjector {
    ...
    /// A hotkey's target comes from `frontmostApplication`, which can be Onecast itself.
    ...
    func copySelection(
        ...
        clipboardManager.prepareForOnecastPasteboardMutation()
    ...
    private func restore(_ snapshot: PasteboardSnapshot, to pasteboard: any PasteboardAccess) {
        ...
        clipboardManager.synchronizeAfterOnecastPasteboardMutation(
    ...
    private func finish(_ lease: TemporaryPasteboardLease) {
        ...
        case .restored(let changeCount):
            ...
            clipboardManager.synchronizeAfterOnecastPasteboardMutation(changeCount: changeCount)
        case .superseded:
            break
        case .failed:
