...
extension UTType {
    ...
    static let onecastBackup = UTType(exportedAs: "com.onecast.backup")
}
...
enum BackupActions {
    ...
    // MARK: - Onecast native (own file panels; dialogs come from `AppCore`)
    ...
    static func chooseBackupFile() -> URL? { chooseFile(ofType: .onecastBackup) }
    ...
    {
        guard let destination = chooseSaveLocation(named: "Onecast", type: .onecastBackup) else {
    ...
    private static let restartAfterImportText = "Quit and reopen Onecast to finish."
