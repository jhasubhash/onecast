...
enum Paster {
    /// Stamped on Onecast's own synthetic keystrokes so the snippet keyword tap can skip them.
    ...
    private static func postCommand(key: CGKeyCode, toPid pid: pid_t?) {
        ...
        down.setIntegerValueField(.eventSourceUserData, value: onecastEventTag)
        up.setIntegerValueField(.eventSourceUserData, value: onecastEventTag)
