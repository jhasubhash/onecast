...
enum ShellCommandRunner {
    ...
    private static let queue = DispatchQueue(
        label: "com.onecast.shell-command", qos: .userInitiated, attributes: .concurrent)
    ...
    nonisolated private static func execute(
        ...
        // Lets a shell config skip slow sections when Onecast is the caller.
        process.environment = ProcessInfo.processInfo.environment.merging(["ONECAST": "1"]) { _, new in
            new
        }
    ...
    nonisolated private static func shellArguments(
        ...
        [loadingShellEnvironment ? "-ilc" : "-lc", command, "onecast"] + arguments
    }
    ...
    private final class StreamCapture: @unchecked Sendable {
        ...
        static func make() -> StreamCapture? {
            let template = FileManager.default.temporaryDirectory
                .appendingPathComponent("onecast-command-stream.XXXXXX").path
