...
private enum CapsLockRemap {
    ...
    private static let queue = DispatchQueue(label: "com.onecast.capslock-remap", qos: .utility)
    ...
    private static func apply(_ mapping: String) {
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                NSLog("Onecast: hidutil remap exited %d", process.terminationStatus)
            }
        } catch {
            NSLog("Onecast: hidutil caps lock remap failed: %@", error.localizedDescription)
        }
    }
}
