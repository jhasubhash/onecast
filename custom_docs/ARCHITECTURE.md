...
How Onecast is built, for **this fork**: enough to navigate the code and add what you need without
...
```
OnecastApp (@main)          two MenuBarExtra scenes, nothing else declarative
...
(`grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' Onecast/Features/*/Model/` must be empty).
...
`OnecastApp` (`@main`) declares **only two `MenuBarExtra` scenes**; everything else is imperative
...
- The **footer** is the shared **`ActionBar`** from `OnecastPluginKit` —
...
| Isolation | a **capability bridge**, not a sandbox — JS reaches Swift only through the `__onecastHost` seam and only JSON data crosses back | none — calls Swift directly |
...
- **`Features/Extensions/`** — `ExtensionManager` (installed set, session state, launcher publishing)
  + `ExtensionRuntime` (the `JSContext` + `__onecastHost`) + `ExtensionNodeShims` (fs/proc/crypto
  surface). No Swift object crosses back to JS — only JSON.
- **`OnecastPluginKit/`** — the public framework contract, embedded for exact-module version
...
1. **A JS extension** (`~/Developer/onecast_addons/`) — zero host change, zero rebase surface.
   Default home for "a thing I want Onecast to do"; see [§9](#9-third-party-code-two-systems) for
   the trust trade-off.
2. **A native plugin** (also in `onecast_addons`) — full Swift power via `OnecastPluginKit`, still
