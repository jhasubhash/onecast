...
Onecast/Features/AI/Model/{Assistant,Skill,SkillFrontmatter,AssistantOpenPolicy}.swift
Onecast/Features/AI/Service/{AssistantStore,SkillStore}.swift
Onecast/Features/AI/Settings/{AssistantsSettingsSection,AssistantEditorSheet,SkillsSettingsSection}.swift
Tests/assistant-test.swift
Onecast_addons: a sample AssistantPlugin (lives outside this repo, see CUSTOM.md)
```
...
OnecastPluginKit/OnecastPluginKit.swift  AssistantPlugin, AssistantDescriptor, AssistantSession, …
...
`OnecastPluginKit.framework`.

### 19.1 The contract (additions to `OnecastPluginKit`)

A plugin opts in by also conforming to `AssistantPlugin`, which extends the existing `OnecastPlugin`:

```swift
@MainActor
public protocol AssistantPlugin: OnecastPlugin {
...
- A plugin wanting a fully bespoke experience can ignore `send` and render anything — being an
  ...
  copy, like `OnecastPlugin` (§19.6): `AssistantSession`'s types are the same across the `dlopen`
  boundary.
...
A plugin assistant is native code in Onecast's process — unsandboxed, full privileges, the existing
...
- **The bridge is the framework.** A plugin talks to the AI only through `AssistantSession`; it never
  imports or links the host's AI types. One copy of the contract, like `OnecastPlugin`.
...
- **Phase 8 — contract:** `AssistantPlugin`, `AssistantDescriptor`, `AssistantSession`,
  `AssistantMessage`, `AssistantModel`, `AssistantTool` in `OnecastPluginKit`; a host
  `AssistantSessionBridge` implementing the protocol over `AIChatState`/`AIChatCoordinator`; a sample
  assistant plugin in `onecast_addons`.
