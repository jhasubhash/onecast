...
OpenSSL 3's default MAC is unreadable by `security import`. The `Onecast Self-Signed` identity was
...
xcodebuild -project Onecast.xcodeproj -scheme Onecast -configuration Debug \
    -derivedDataPath build/DerivedData build
open "build/DerivedData/Build/Products/Debug/Onecast Dev.app"
```

Or `open Onecast.xcodeproj` and ⌘R. Debug is its own channel — `Onecast Dev.app`,
`com.onecast.app.dev`, with its own prefs, caches and TCC grants — so it can't see or clobber an
installed Onecast. It starts with no hotkeys bound; record one in Settings → General.

**After a rebuild, quit and relaunch a running `Onecast Dev` — a stale instance is not harmless.**
...
```sh
osascript -e 'quit app "Onecast Dev"'
open "build/DerivedData/Build/Products/Debug/Onecast Dev.app"
```

**Debug and Release both sign with a real Developer ID, not `Onecast Self-Signed`.** The identity is
never committed: `Signing.xcconfig` (the base config for both configs) sets the portable
`Onecast Self-Signed` default and ends with `#include? "Signing.local.xcconfig"` — a gitignored file
...
```sh
codesign -dvv "build/DerivedData/Build/Products/Debug/Onecast Dev.app" 2>&1 | grep Authority
...
- **JS extensions** each build with one esbuild line in their `package.json`;
  [`copy-path`'s README](../../onecast_addons/extensions/copy-path/README.md) is the reference build
...
- **Native plugins** (see [plugins.md](plugins.md)) also live there — `hello` is the reference — but
  each is Swift the app compiles from source against its embedded `OnecastPluginKit`, installed as
  `manifest.json` + `.swift` sources to `~/Library/Application Support/<bundle id>/plugins/<name>/`.
...
"$TMPDIR/onecast-harness/ext-test" ~/Developer/onecast_addons/extensions/<name> <command>
```

Reach for a change in *this* repo only when the host itself is wrong — a `@raycast/api` component
Onecast renders badly, a missing Node shim, a runtime bug. Native plugin support *did* require host
...
4. **`project.yml` is the source of truth.** After editing it run `xcodegen generate` and commit
   `Onecast.xcodeproj` too — editing the `.xcodeproj` directly is lost on the next generate and is a
...
| Native Swift/SwiftUI plugin system | `OnecastPluginKit/`, `Features/Plugins/**`, `App/AppCore.swift`, `Palette/{PaletteMode,PaletteEscapeAction,RootPaletteView}.swift`, `Features/Launcher/{Service/AppIndex,Service/VisibilityStore,UI/LauncherList,UI/LauncherCoordinator}.swift`, `Features/HotKeys/{Model/HotKeyAction,Service/HotKeyManager}.swift`, `Features/Settings/{SettingsTab,SettingsDetailView,AppSettings,AppSettingsKey,SettingsSearchCatalog}.swift`, `Features/Backup/Model/SettingsBackupCoverage.swift`, `project.yml`, `Onecast.entitlements` | Compiled Swift plugins that `dlopen` into the palette and render SwiftUI, like BetterTouchTool's Swift plugins; complements the JS extension system. A plugin links the embedded `OnecastPluginKit.framework` for one shared type identity across the load boundary. | Maybe — needs the `disable-library-validation` entitlement, which upstream may decline. |
| Plugins compiled from source by the app | `Features/Plugins/Service/{PluginBuilder,PluginCatalog,PluginLoader,PluginManager}.swift`, `Features/Plugins/UI/PluginWindowController.swift`, `Features/Settings/SettingsSearchCatalog.swift`, `project.yml`, `Tests/plugin-catalog-test.swift`, `docs/plugins_and_extensions.md`, `custom_docs/plugins.md` | A plugin is now just `manifest.json` + `.swift` beside it: `PluginBuilder` compiles it with `swiftc` (through `xcrun`) against the app's own embedded `OnecastPluginKit`, ad-hoc signs and caches the dylib under `Caches/<id>/PluginBuilds/` keyed on a source fingerprint — no author build step, no `build.sh`, no build-products framework. A `project.yml` post-build step restores the `.swiftinterface` Xcode strips on embed (and re-signs) so the shipped app can compile against itself; the folder watcher rebuilds on edit and the content-addressed loader maps fresh bytes with no relaunch. `manifest.json` dropped `dylib`, gained optional `module`. | No |
| Pin/unpin a plugin's nested view to the launcher (Add ⇄ Remove from Main Menu) + copy its deep link | `OnecastPluginKit/` (`PluginRoute`, `PluginContext.route`, scaffold `route:`, `PluginCommand.mainMenuSlot()` for placement, `pluginToggleMainMenu`/`pluginMainMenuPinned`/`pluginCopyRouteLink` env hooks), `Features/Plugins/{Service/PluginManager,UI/PluginCoordinator}.swift` (route through launch/context, `PluginRouteURL`, `toggleRoutePin`/`isRoutePinned`/`copyRouteLink`, uninstall prunes pins), `Features/Quicklinks/UI/QuicklinkCoordinator.swift` (intercept `onecast://plugin/…`), `Palette/RootPaletteView.swift` (inject the hooks) | A saved deep link, stored as an ordinary Quicklink so it inherits aliasing, shortcuts and backup, opens a plugin straight to a nested view. The ⌘K palette offers Add/Remove-from-Main-Menu and "Copy Deep Link" on any surface that supplies a `PluginRoute` — placed where the plugin drops a `mainMenuSlot()`, else appended — keyed on the route link (not the name) so a renamed pin still toggles; uninstalling the plugin removes its pins. Generic seam; plugins support arbitrary views, extensions can adopt command+args later. | Yes |
...
| Debug signs with a real Developer ID identity, via a gitignored local xcconfig | `Signing.xcconfig` (new — project Debug base config), `Signing.local.xcconfig` (new — gitignored), `project.yml`, `.gitignore`, `Onecast.xcodeproj` | The login-Keychain "Always Allow" grant (AI keys, MCP, `assistant-environment`) only survives a rebuild when the app's code-sign anchor is Apple-trusted; `Onecast Self-Signed` is `CSSMERR_TP_NOT_TRUSTED`, so the Keychain reprompted on every rebuild while TCC — matching by DR — did not. Debug now signs with `Developer ID Application: … (<TEAM_ID>)`, a team-pinned DR stable across rebuilds and cert renewal. The identity is **not** committed: `Signing.xcconfig` carries the portable `Onecast Self-Signed` default and `#include?`s the gitignored `Signing.local.xcconfig` that holds the personal identity, so any clone (and CI) builds self-signed and unaffected. (Release now shares this via the row below.) | Mechanism yes; identity machine-local |
