...
**Onecast targets one macOS — the current stable release — and nothing else.** macOS 26+, the Xcode 26
...
| Folder | Holds |
| --- | --- |
| `Onecast/App/` | `@main`, `AppDelegate`, `AppCore` — the composition root |
| `Onecast/DesignSystem/` | shared visual primitives; `Theme.swift` is the only design-token source |
| `Onecast/Platform/` | system shims: `Permissions`, `AppPaths`, `Signposts`, `NotificationToken`, … |
| `Onecast/Palette/` | the palette shell: panel, window controller, `RootPaletteView`, `PaletteScreen` |
| `Onecast/Windows/` | the non-palette AppKit surfaces: `Dialog/`, `HUD/`, `About/`, `AppWindowController` |
| `Onecast/Features/` | one folder per feature; larger ones split `Model/` `Service/` `UI/` `Settings/` |
...
- **Onecast presents its own dialogs — never `NSAlert`, `NSSlider` or a system popover.** A question
...
- **Debug builds are their own channel** — `Onecast Dev.app` / `com.onecast.app.dev` — so a local run
...
- **XcodeGen owns the project.** `Onecast.xcodeproj` is committed but generated from `project.yml`;
...
- `grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' Onecast/Features/*/Model/` returns nothing.
