...
How to check that a change holds up. Onecast has no XCTest target and no UI test suite: the
...
| Pure-layer purity | `grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' Onecast/Features/*/Model/` |
...
seconds to about 15. `ONECAST_TEST_JOBS=1` forces it back to one at a time. Each result is numbered
...
than `ONECAST_TEST_TIMEOUT` seconds (default 300) is killed and reported as timed out. Parallelism is safe
...
is the trap: a running Onecast records every write to it as a genuine copy, so a fixture left there
...
```sh
grep -rln 'import AppKit\|import SwiftUI\|import Cocoa' Onecast/Features/*/Model/
```
...
xcodebuild build -project Onecast.xcodeproj -scheme Onecast -configuration Debug \
  CODE_SIGNING_ALLOWED=NO
xcodebuild build -project Onecast.xcodeproj -scheme Onecast -configuration Release \
  CODE_SIGNING_ALLOWED=NO
find ~/Library/Developer/Xcode/DerivedData -name "Onecast*.app" -maxdepth 6 -print -quit
```
...
`Platform/Signposts.swift` emits eight intervals on the `com.onecast.perf` subsystem: `AppCore.start`,
...
```sh
swiftc -O -swift-version 6 Onecast/Platform/Signposts.swift \
...
```sh
swiftc -O -swift-version 6 Onecast/Features/Calculator/Model/*.swift \
...
```sh
swiftc -O -swift-version 6 Onecast/Platform/PasteboardFiles.swift \
    Onecast/Features/Clipboard/Model/{ClipboardStore,ClipboardFilter,ColorValue,ColorFormat,ColorSpaces}.swift \
...
swiftc -O -swift-version 6 Onecast/Features/Emoji/Model/{EmojiCatalog,EmojiData.generated}.swift \
    Onecast/Features/Emoji/Service/{EmojiIndex,FrequentEmojiStore}.swift \
...
Run against the **Debug channel** (`Onecast Dev.app`, `com.onecast.app.dev`). It has its own prefs,
...
- Paste from clipboard history lands in that app, not in Onecast
...
- A shortcut that fails shows Onecast's dialog with the tool's error
...
- Delete confirms through Onecast, moves the file to Trash, and selecting another note never loses an
  unsaved edit
...
- Cancelling the save panel leaves nothing in `~/Library/Caches/com.onecast.app.dev/backup-staging/`
...
- Nothing was written outside `com.onecast.app.dev/`. Channel isolation is not negotiable — a Dev build
