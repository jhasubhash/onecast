...
Export and import of Onecast's own data as a single `.onecast` file, plus the entry point for
...
- **No absolute path may enter a `.onecast`.** A clip's `imagePath` names a file on the Mac that wrote
...
| `Model/BackupArchive.swift` | Directory ⇄ `.onecast`; the only file importing `AppleArchive` |
...
| `Service/RaycastImportReader.swift` | Raycast → Onecast field mapping |
...
The old flat `Onecast-Settings-*.json` export is gone rather than deprecated, and nothing reads it.
