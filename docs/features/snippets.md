...
Debug (`com.onecast.app.dev`), beta, and stable therefore never share snippet files. The storage
...
frontmatter keeps the same identity. Renaming a file outside Onecast appears as deletion of the old
...
an independently selectable category. Onecast reads `title`, `text` and the optional `keyword` from
...
suffixes as snippets created in Onecast, and duplicate keywords are preserved.
...
there are no aliases, so a key Onecast does not know names itself in the error.
...
A token Onecast cannot parse — an unknown name, an unknown modifier, a duplicated or unsupported
...
duplicates resolve by file identity. Onecast-tagged synthetic events are ignored.
...
After either launcher or keyword delivery is confirmed, Onecast may show a brief non-activating,
...
for permissions or activates Onecast. Failed, cancelled, rejected, or prompt-cancelled expansions do
...
Longer or multiline fallback text uses a temporary paste. Onecast snapshots every item, type and data
payload, then **lends a board holding nothing but the expansion** — one item carrying the plain text
and Onecast's own marker type — and restores by rewriting the snapshot whole.
...
previous copy instead. A representation Onecast cannot rewrite to mean the expansion is one it must
...
synchronizes to Onecast's ownership changes so temporary or restored text is not added as new history.
...
any pasteboard restoration still owned by Onecast.
...
non-ignored input to `cancelAutomaticExpansion`, and Onecast's own tagged synthetic events classify
...
between revalidation and mutation, so Onecast does not claim that impossible guarantee.
