...
Stats, Onecast itself, Mission Control, Siri, Time Machine, Screenshot, System Information, Font
...
- **Apps win the ties.** `KindDescriptor.rankPriority` puts applications (4) above command-like kinds
  ...
  shadowed by the Onecast command named after it: Calculator over Calculator History.
...
`Onecast Settings`, like About, Quit and Support Onecast, so nothing ties it there.
...
and its `url` carries the destination instead of the catalog's `onecast://` placeholder. Nothing
...
`SystemActionCatalog` is a Foundation-only inventory of the macOS actions Onecast exposes. Its
...
Toggle System Appearance changes macOS; Onecast follows it only while its own Appearance is System.

Restart, Shut Down, Log Out, Empty Trash and Quit All Applications confirm before execution: ↵ runs
the action, Escape cancels. Every dialog is Onecast's own: confirmations, failure reports and the Set
...
Onecast. See [quick-actions.md](quick-actions.md).
...
for Updates, Onecast Settings, About, Support and Quit.
...
- **Quit All Applications** a system action. `AppLauncher.quitAllTargets()` is the
  policy (every `.regular` app except Finder — `terminate()` only relaunches it — and Onecast,
