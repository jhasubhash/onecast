...
- **Search is filename-only, and every list comes from Spotlight.** Onecast creates no content index,
  ...
  `kMDItemFSContentChangeDate`, never from anything Onecast recorded. The type filter narrows *which*
...
- **`~/Library` is never a scope Onecast picks by itself.** A configured home root expands into its
...
- **Onecast asks for no file permission.** Hidden metadata items and application bundles are filtered,
...
`com.onecast.perf` signpost subsystem. `Tests/file-search-performance.swift` exercises the same service
...
half the folder hits on a developer machine are some `src` or `Onecast`. The path itself is the preview's
...
ordinary settings carried by Onecast settings backups; importing them grants no permission or
background access.
