...
- **One funnel, whichever way an action started.** A shortcut and a launcher row both land on
  ...
  palette — once the palette is gone, the frontmost app is Onecast, and the action would read its
...
- **Onecast is never an event target.** `QuickActionRunner.selection(in:using:)` refuses our own
...
A custom action previews by default, switchable to Replace per row: Onecast cannot know whether an
...
`HotKeyManager.start` can prune a binding whose action was deleted while Onecast was off.
...
`QuickActionPanel` is Onecast's **fourth borderless surface**, beside the dialog, the notes panel
...
Onecast asserts, so it is the part worth checking by hand.
...
- Run one from the launcher (⌘Space → "Fix Grammar") with text selected behind it: the palette
  closes and the selection in the displaced app is what gets acted on, not Onecast's own field.
...
- Press a shortcut with Onecast's own Settings window frontmost: refused, with a HUD.
