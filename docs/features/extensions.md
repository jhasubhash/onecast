...
Onecast runs Raycast extensions: the same `package.json` + prebuilt CommonJS bundles Raycast itself
...
`react/jsx-runtime`, `@raycast/api` and the Node built-ins external. Onecast supplies exactly those,
...
`Onecast/Resources/RaycastRuntime.generated.js` (~200 KB minified) is **generated and committed**, the
same arrangement as `EmojiData.generated.swift`: building Onecast never needs Node. Sources live in
[`Scripts/raycast-runtime/`](../../Scripts/raycast-runtime):

| File | What it is |
| --- | --- |
| `src/index.js` | the `__onecast` object Swift calls into (`boot`, `start`, `dispatch`, `popNavigation`, `settle`, `fireTimer`, `stop`) |
...
- **Async** (`invoke`) for anything that needs the main actor — clipboard, toasts, window control,
  `fetch`, `exec`, `oauth`. Swift answers later through `__onecast.settle`, so the JS thread never blocks on the
  UI.
...
`Onecast/Features/Extensions/`, split the same way as every other feature:
...
- **Search-bar dropdown** — `List.Dropdown` and `Grid.Dropdown` draw as
  ...
  command's own dropdown answers before Onecast's clipboard filter can. The list is
...
- **Form** — label-left/control-right rows. Field values live in the extension (React owns them); every
  edit dispatches `onOnecastChange` and the resulting re-render is what updates the control, so
...
2. **Import from Raycast** — copies the already-built bundles out of a local Raycast. Nothing is
   ...
   so when Raycast has something Onecast doesn't — installing in Raycast otherwise leaves no trace
...
a browser link, another app, a Shortcut — and `onecast://` mirrors it so our own links never depend
...
tokens in the login Keychain (service `com.onecast.extensions.oauth`, `kSecAttrAccessibleWhenUnlocked`),
scoped per extension and dropped on uninstall.

The redirect address belongs to the extension author's OAuth app registration, so Onecast cannot choose
it — it can only be there to catch it. **Onecast therefore claims `raycast`, `com.raycast` and `onecast`
...
| --- | --- | --- |
| `App` | `raycast://oauth?package_name=Extension` | straight to Onecast, no server |
| `AppURI` | `com.raycast:/oauth?package_name=Extension` | straight to Onecast, no server |
| `Web` | `https://raycast.com/redirect?packageName=Extension` | through Raycast's page, which reopens a claimed scheme |

Claiming `raycast` means an installed Raycast competes with Onecast for those links and macOS picks the
...
this). `ExtensionHostBridge` keeps those inside Onecast: `raycast://extensions/<author>/<extension>/<command>`
...
bundle that merely references `dgram` or `http2` still loads. Those stubs are manufactured lazily,
...
host rather than returning a wrong path. Node's `windows` override is absent: Onecast only runs on
...
node build.mjs            # → Onecast/Resources/RaycastRuntime.generated.js (commit it)
...
"${TMPDIR:-/tmp}"/onecast-harness/ext-test ~/Library/Application\ Support/com.onecast.app.dev/extensions/<name> [command]
```
...
```sh
EXT_TEST_MENU_BAR=1 "${TMPDIR:-/tmp}/onecast-harness/ext-test" \
  "$HOME/Library/Application Support/com.onecast.app.dev/extensions/opencodex-usage" usage-menu-bar
```
...
| OAuth tokens | macOS Keychain (`com.onecast.extensions.oauth`) | yes |
...
**Builds happen in `$TMPDIR/onecast-install-<UUID>/`**, named by `ExtensionCleanup.workspace` so the
