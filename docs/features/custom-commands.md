...
all. `ONECAST=1` exists so an rc file can skip those sections: `[[ -n $ONECAST ]] && return`.
...
file reading `/dev/tty` can hang there but not for real users. There is **no timeout** — Onecast never kills a
running command except through the output window's Stop button, and a command outlives Onecast
quitting.
...
```
/bin/zsh -lc '<command>' onecast <value1> <value2> …
```
...
user wrote themselves wants a deliberate second tap, not a red alarm. The gate is Onecast's own
...
Onecast dismisses an open palette before starting a custom command. With **Show output** off, a zero
exit status is silent; a launch failure or non-zero status opens a Onecast dialog with the bounded
