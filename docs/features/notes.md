...
- **The editor is the one surface snippets expand into.** `NoteTextView` adopts `InjectableTextView`,
  ...
  rather than posted as events at whichever app happens to be frontmost. Nothing else in Onecast
...
restores the prior external application or Onecast window and flushes without delaying the order-out —
...
Command-Q is bound to nothing app-wide, so no chord over Notes can quit Onecast.
...
`NoteEditorInput.source` directly as `NSTextView.string` with one system font and Onecast's note color.
