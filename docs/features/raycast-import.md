...
Onecast reads the `.rayconfig` written by Raycast v2.x: a `RAYCFG3` container holding an AES-256-GCM
...
Extensions → Export Settings & Data. **Onecast never reads the keychain**; the user supplies the
passphrase.
...
those into Onecast's domain types. That is the same pure-layer / platform-layer split
