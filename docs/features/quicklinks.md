...
Quicklinks reuse Onecast's one template engine — the same
...
Onecast's own dialog and leaves no partial state.
...
independently selectable category. Onecast reads `name`, `link`, `createdAt` and the optional
