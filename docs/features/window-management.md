...
Rule 1 also delivers the "works for windows Onecast never moved" requirement: the capture happens in
...
Our panels are `.nonactivatingPanel`, so opening one never makes Onecast frontmost and
...
This mirrors `InjectionTarget` in [Snippets](snippets.md), which solved the same problem for
keystrokes: same shape, same `Service/` position, one idea applied twice.
...
- **The Dock ignores a gesture from a short-lived process.** The calls all report success and nothing
  happens. Onecast is a resident menu-bar app, so this is free — but it is why a one-shot CLI cannot
...
5. Restore on a window Onecast has never moved, and after a custom size.
