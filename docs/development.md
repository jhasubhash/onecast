...
Create the `Onecast Self-Signed` code-signing identity once — builds sign with it, which is what keeps
...
```sh
open Onecast.xcodeproj    # then ⌘R
```

Or from the command line:

```sh
xcodebuild -project Onecast.xcodeproj -scheme Onecast -configuration Debug build
```
...
`Onecast.xcodeproj` is committed and generated from `project.yml` via XcodeGen — after changing
...
Debug builds are a separate channel: **`Onecast Dev.app`**, bundle id `com.onecast.app.dev`. Every
...
- The dev build asks for Accessibility on its own the first time, and starts with **no** hotkeys bound
  ...
  and the `Onecast Self-Signed` identity keep the TCC grant alive.
...
xcodebuild -project Onecast.xcodeproj -scheme Onecast -configuration Debug \
    -derivedDataPath build/DerivedData build 2>&1 | tee /tmp/onecast-build.log
./Scripts/sync-lsp.sh /tmp/onecast-build.log
```
...
**Build Onecast.app (debug)** task (⌘⇧B) and **F5** re-run the script on every build, so new and
...
```sh
./Scripts/format.sh            # format Onecast/ and Tests/ in place
...
```sh
node Scripts/gen-emoji.js            # -> Onecast/Features/Emoji/Model/EmojiData.generated.swift
node Scripts/gen-currencies.js       # -> Onecast/Features/Calculator/Model/CurrencyData.generated.swift
node Scripts/gen-countries.js        # -> Onecast/Features/Calculator/Model/CountryZoneData.generated.swift
```
