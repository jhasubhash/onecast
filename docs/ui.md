...
The design system for Onecast's UI, written so an agent restyling or extending it stays consistent
with what's already there. This documents **Onecast as built** — every rule here maps to code in
`Onecast/`. `DesignSystem/Theme.swift` is the single design-token source.
...
Onecast is a **command palette**: a borderless floating panel whose surface is just the
...
Source: `Onecast/DesignSystem/Theme.swift`.
...
Onecast owns its dialogs; `NSAlert` is never used. `DialogController` is owned by `AppCore` (the
...
writes `AppleShowScrollBars = WhenScrolling` into Onecast's own defaults domain, which outranks the
...
countdown and buttons around it are Onecast's own. Its buttons are a **deliberate copy** of
...
Its placeholder is drawn by Onecast, not by the field's `prompt` — an `NSTextField` renders a prompt
...
[features/palette.md](features/palette.md#the-placeholder-is-onecasts-not-the-fields).
