...
/// A native Onecast plugin. Conform an `NSObject` subclass or a plain `final class`, expose it
/// through a `@_cdecl("onecastPluginCreate")` entry point (see `OnecastPluginRuntime.export`),
...
@MainActor
public protocol OnecastPlugin: AnyObject {
    init()
...
public extension OnecastPlugin {
...
/// named `onecastPluginCreate` with this type.
public typealias OnecastPluginCreate = @convention(c) () -> UnsafeMutableRawPointer

/// The bridge between a plugin's `@_cdecl` entry point and the host loader.
public enum OnecastPluginRuntime {
    ...
    /// plugin here is safe.
    public static func export(_ make: @MainActor () -> any OnecastPlugin) -> UnsafeMutableRawPointer {
    ...
    public static func consume(_ pointer: UnsafeMutableRawPointer) -> (any OnecastPlugin)? {
        Unmanaged<AnyObject>.fromOpaque(pointer).takeRetainedValue() as? any OnecastPlugin
    }
}
...
public struct PluginCommand: Identifiable {
    ...
    public static let mainMenuSlotID = "__onecast_main_menu_slot__"
    ...
    private var currentCommands: [PluginCommand] {
        ...
        let routeCommands: [PluginCommand] = route().map { route in
            ...
            return [
                ...
                PluginCommand(
                    id: "__onecast_copy_deep_link__",
                    title: "Copy Deep Link",
                    icon: .symbol("link"),
                    action: { pluginCopyRouteLink(route) }),
                PluginCommand(
                    id: "__onecast_pop_out__",
                    title: "Pop Out",
