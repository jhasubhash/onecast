...
Swedish and Icelandic, so Onecast will not guess.
...
Restarting Onecast never fetches again early, so opening it with a fresh copy makes **zero**
network requests.
...
It all works offline. Onecast does not look cities up online.
