...
<kbd>⌘</kbd><kbd>Q</kbd> does nothing here, so **no key press over Notes can quit Onecast** by
accident.

## Files

Notes live in Onecast's Application Support folder:

```
~/Library/Application Support/com.onecast.app/Notes/
```
...
**A save overwrites whatever is on disk.** Onecast does not watch the folder. If you edit the note
that is _open in Onecast_ in another app, that edit is lost at the next autosave.
