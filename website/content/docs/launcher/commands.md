...
Commands are things Onecast does, reachable by name from the launcher. Some are built in. Others
...
| Commands                                           | Calculator History · [Open Camera](/docs/features/camera) · Export Backup · Import Backup · Import from Raycast · Check for Updates · Settings · About Onecast · Support Onecast · Quit Onecast |
...
which need text to work on, and Quit Onecast, so no stray key press can quit the app.
...
| Environment | Inherited, plus `ONECAST=1`                        |

**There is no Terminal window and no timeout.** Onecast never stops a running command on its own,
and a command keeps running if Onecast quits. The one exception is the **Stop** button in the output
window.

If the Run In folder no longer exists, the command does not run, and Onecast tells you which folder
...
`ONECAST=1` lets your `.zshrc` skip that work:
...
`ONECAST=1` check above keeps it quiet.
