...
switched off in System Settings, Onecast tells you so. **It never quietly moves you to a paid API
instead.**

### Installed AI: Codex, Claude and OpenCode

If you already use the `codex`, `claude` or `opencode` command-line tools, Onecast can use them
with the account you are signed in to. **Onecast never asks for or stores their keys.**
...
Onecast uses them as plain chat. Their tools, file access and shell access are switched off, and
...
- A key belongs to the address it was saved for. Point a connection at a different base URL and
  Onecast asks for a new key rather than sending the old one somewhere new.
- While you edit a connection, Onecast asks the provider which models your key can use, and you
...
Onecast sends a short note ahead of every conversation that tells the model where it is running.
...
- Conversations are saved on your Mac in `ai-chats.sqlite3`, in Onecast's Application Support
  folder.
