---
title: What works
description: What Onecast supports from the Raycast API, how well it works in practice, and the known gaps.
---
...
To catch the redirect, Onecast registers the `raycast://` link type. If Raycast is also installed,
...
**`raycast://` links** stay inside Onecast. A link to an installed extension command runs that
command — from another app or the browser as well as from inside an extension — with its `arguments`,
`fallbackText` and `launchType` honored, and `onecast://` works the same way. Anything else reopens
