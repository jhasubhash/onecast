...
Onecast looks in both `~/.config/raycast` and `~/.config/raycast-x`, and an extension found in both
is offered once. There is an **Import All** button. The pane checks again whenever you open it, and
tells you when Raycast has something Onecast does not.
...
An app opened from the Dock does not see your terminal's `PATH`, so Onecast looks in the usual places
...
Everything lives in Onecast's Application Support folder, and **uninstalling an extension removes
