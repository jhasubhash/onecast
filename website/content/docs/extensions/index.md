---
title: Extensions
description: Onecast runs Raycast extensions natively, drawn with SwiftUI.
---

Onecast runs Raycast extensions: the same `package.json` and the same built command files, drawn
natively in the palette.

There is no Electron, no browser and no Node.js running inside Onecast. Extensions run in
...
leave it.** That is the only ongoing cost Onecast has.
