---
title: Updates
description: How Onecast finds, checks and installs new versions by itself.
---

Onecast keeps itself up to date. There is nothing to set up.

## How it works

1. **Once a day**, Onecast asks GitHub whether a newer release is out for its channel. If a check
...
4. Onecast checks the download, installs it, and offers to **relaunch**.
...
launcher, the Onecast menu bar menu, and **Settings → About**.
...
On an Intel Mac, Onecast only installs the universal build. If a release has none, nothing is
...
- **The signature must prove the download is Onecast**, signed the same way as the copy you are
  running.
- The app identifier and the version must be the ones expected.
- Onecast never runs `xattr` or asks for an administrator password. If `/Applications` cannot be
...
Onecast's Homebrew casks tell Homebrew that the app updates itself. So `brew upgrade` **skips
Onecast on purpose**: it never reports it as outdated, never downloads it again, and never rolls a
...
Onecast is free and open source. **Support Onecast** in the launcher, the menu bar and
