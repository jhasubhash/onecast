...
Onecast reads Raycast's own export file directly, in **Settings → Backup → Raycast Export**, or in
the welcome screen the first time you open Onecast.

## Which exports it reads

**Raycast v2.0 and newer only.** Onecast reads the `.rayconfig` file that Raycast v2.0 and later
write, and nothing else. Check your Raycast version in **Raycast → Settings → About** before you
export.

**Raycast v1.x exports are not supported.** That format was dropped in **Onecast v0.10.5**, along with
...
2. In Onecast, go to **Settings → Backup → Raycast Export** and choose the file.
3. Type the passphrase, tick the things you want, and import.
4. **Quit Onecast and open it again.**

**Quit and reopen Onecast once the import finishes.** Not all of what you brought across takes effect
in the running app, so a full restart is what makes the whole import live. Quit from the menu-bar icon
— closing the Settings window is not enough — then open Onecast again.

Onecast recognizes the file **before** you type the passphrase, so a wrong passphrase is reported as
...
**Onecast never reads your Keychain.** You paste the passphrase in yourself.
...
| Emoji skin tone     | Raycast's default becomes Onecast's Default               |
| Compact mode        | From Raycast's window mode                                 |
| Pop to root         | Only when the timing matches one Onecast offers           |
| Launch at login     |                                                            |
...
A shortcut with a modifier Onecast does not recognize is **skipped whole**, rather than imported as
...
Nothing in Onecast needs Raycast to be installed, except importing extensions from it, which by
