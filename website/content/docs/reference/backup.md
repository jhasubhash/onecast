...
A backup is one `.onecast` file. You tick what goes into it, and you tick again what comes back out
...
**The format belongs to Onecast and may change between versions. The only promise is that a backup
imports into the same version that made it.**

The file records which version wrote it. A Onecast that does not recognize it says so clearly,
...
Everything a backup carries is also an ordinary file, in `~/Library/Application Support/com.onecast.app/`:
...
read them without Onecast. That is the point of keeping them that way.
