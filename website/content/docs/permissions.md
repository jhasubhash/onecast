---
title: Permissions
description: What Onecast asks for, why, and the moment it asks.
---

Onecast asks for a permission **only when you use a feature that needs it**, never at launch. The
...
macOS calls this "control your computer". In Onecast it lets the app read from, and write to, the
...
- Onecast uses a **listen-only** event tap under the Accessibility grant. It does not use Input
  Monitoring.
...
Turning it on shows Onecast's own explanation first, then the macOS prompt. The feature only counts
...
If you say no, Onecast tells you and links to the right System Settings pane instead of silently
doing nothing.

## Full Disk Access

Onecast **checks** for Full Disk Access but **never asks** for it.
