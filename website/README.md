...
The marketing page and documentation for Onecast, at
<https://onesyntax.in/onecast/>.
...
```sh
npm install
npm run dev      # http://localhost:3000/onecast
```
...
The site is served from the `/onecast/` subpath, set as `basePath` in `next.config.mjs`. `next/link`
...
```sh
npm run build
mkdir -p /tmp/pages && cp -r out /tmp/pages/onecast
cd /tmp/pages && python3 -m http.server 4321
# http://localhost:4321/onecast/
```
