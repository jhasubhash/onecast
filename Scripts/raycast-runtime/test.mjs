...
const runtimePath = [
  resolve("Onecast/Resources/RaycastRuntime.generated.js"),
  resolve("../../Onecast/Resources/RaycastRuntime.generated.js"),
  fileURLToPath(new URL("../../Onecast/Resources/RaycastRuntime.generated.js", import.meta.url)),
].find(existsSync);
...
export function createHarness({ onRender, onFail, verbose = false, stubs = {} } = {}) {
  ...
  const host = {
    ...
    startTimer(id, ms, repeats) {
      const fire = () => runInContext(`__onecast.fireTimer(${JSON.stringify(id)})`, context);
  ...
  function settle(callId, ok, value) {
    runInContext(
      `__onecast.settle(${JSON.stringify(String(callId))}, ${ok}, ${JSON.stringify(value === undefined ? "" : JSON.stringify(value))})`,
      context,
    );
  }

  context.__onecastHost = host;
  // Mirrors what Swift installs: compile the extension's CJS body in global scope.
  context.__onecastCompile = (code, filename) =>
    runInContext(
  ...
  return {
    ...
    boot(config) {
      return runInContext(`__onecast.boot(${JSON.stringify(JSON.stringify(config))})`, context);
    },
    start(sessionId, code, filename, dirname, mode, ctx) {
      return runInContext(
        `__onecast.start(${JSON.stringify(sessionId)}, ${JSON.stringify(code)}, ${JSON.stringify(filename)}, ${JSON.stringify(dirname)}, ${JSON.stringify(mode)}, ${JSON.stringify(JSON.stringify(ctx))})`,
        context,
      );
    },
    dispatch(sessionId, handlerId, args = []) {
      return runInContext(
        `__onecast.dispatch(${JSON.stringify(sessionId)}, ${JSON.stringify(handlerId)}, ${JSON.stringify(JSON.stringify(args))})`,
        context,
      );
    },
    stop(sessionId) {
      runInContext(`__onecast.stop(${JSON.stringify(sessionId)})`, context);
