...
export default function Command() {
  ...
  const parts = [
    ...
    fileURLToPath(pathToFileURL("/tmp/a?b.png")),
    fileURLToPath(pathToFileURL("/Applications/Onecast Beta.app")),
...
export default async function Command() {
  const target = join(tmpdir(), "onecast-fixture-index.json");
...
export async function runFixtures() {
  ...
  await run("Form fields and submit", formSource, "view", async (harness) => {
    ...
    check("field has a change handler", !!field.props.onOnecastChange?.$fn);

    harness.dispatch("s1", field.props.onOnecastChange.$fn, ["Grace"]);
    await wait();
  ...
  await run("Navigation push and pop", navigationSource, "view", async (harness) => {
    ...
    harness.call('__onecast.popNavigation("s1")');
    await wait();
  ...
  `, "menu-bar", async (harness) => {
    ...
    harness.call(`__onecast.dispatch("s1", ${JSON.stringify(item.props.onAction.$fn)}, '[{"type":"right-click"}]', true)`);
