...
class OnecastHeaders {
  constructor(init) {
    this._map = new Map();
    if (init instanceof OnecastHeaders) {
...
class OnecastBlob {
  ...
  slice(start = 0, end = this.size, contentType = "") {
    ...
    return new OnecastBlob([this._bytes.subarray(Math.min(from, to), to)], { type: contentType });
  }
}

function blobPartToBytes(part) {
  if (part instanceof OnecastBlob) return part._bytes;
...
class OnecastFile extends OnecastBlob {
...
class OnecastFormData {
  constructor() {
    this._entries = [];
    // Header and body must carry the same boundary, so it lives on the instance, not the encoder.
    this._boundary = `----OnecastFormBoundary${Math.random().toString(36).slice(2, 18)}`;
  }
...
function formDataValue(value, filename) {
  if (!(value instanceof OnecastBlob)) return String(value);
  if (value instanceof OnecastFile && filename === undefined) return value;
  return new OnecastFile([value], filename ?? "blob", { type: value.type });
}
...
function formDataToBytes(form) {
  ...
  for (const [name, value] of form._entries) {
    const disposition =
      value instanceof OnecastBlob
        ? `; name="${escapeFormName(name)}"; filename="${escapeFormName(value.name)}"`
        : `; name="${escapeFormName(name)}"`;
    const type = value instanceof OnecastBlob ? `Content-Type: ${value.type || "application/octet-stream"}\r\n` : "";
    chunks.push(utf8Encode(`--${form._boundary}\r\nContent-Disposition: form-data${disposition}\r\n${type}\r\n`));
    chunks.push(value instanceof OnecastBlob ? value._bytes : utf8Encode(value));
...
if (!g.Blob) g.Blob = OnecastBlob;
if (!g.File) g.File = OnecastFile;
if (!g.FormData) g.FormData = OnecastFormData;

class OnecastResponse {
  ...
  constructor(body = null, init = {}, url = "") {
    ...
    this.headers = new OnecastHeaders(init.headers);
    this.url = url;
  ...
  clone() {
    ...
    return new OnecastResponse(this._bytes ?? this._stream, { status, statusText, headers }, this.url);
  }
  async arrayBuffer() {
    this.bodyUsed = true;
  ...
  async blob() {
    return new OnecastBlob([await this.bytes()], { type: this.headers.get("content-type") ?? "" });
  }
}

class OnecastRequest {
  constructor(input, init = {}) {
    if (input instanceof OnecastRequest) {
      this.url = input.url;
      this.method = init.method || input.method;
      this.headers = new OnecastHeaders(init.headers || input.headers);
...
async function onecastFetch(input, init = {}) {
  const request = input instanceof OnecastRequest ? input : new OnecastRequest(input, init);
...
class OnecastDOMException extends Error {
...
if (!g.DOMException) g.DOMException = OnecastDOMException;
...
function bodyToBytes(body) {
  ...
  if (body instanceof OnecastBlob) return body._bytes;
  if (body instanceof OnecastFormData) return formDataToBytes(body);
...
function bodyContentType(body) {
  ...
  if (body instanceof OnecastFormData) return `multipart/form-data; boundary=${body._boundary}`;
  if (body instanceof OnecastBlob) return body.type || null;
  return null;
}
...
if (!g.fetch) {
  g.fetch = onecastFetch;
  g.Headers = OnecastHeaders;
  g.Response = OnecastResponse;
  g.Request = OnecastRequest;
}
