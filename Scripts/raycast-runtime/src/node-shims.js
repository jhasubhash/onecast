...
const process = {
  ...
  arch: "arm64",
  version: "v22.0.0",
  versions: { node: "22.0.0", v8: "12.0.0", onecast: "1" },
  argv: ["node", "extension"],
  argv0: "node",
  execArgv: [],
  execPath: "",
  ...
  chdir: () => {
    throw new Error("process.chdir is not supported in Onecast extensions.");
  },
  exit: () => {
    throw new Error("process.exit is not supported in Onecast extensions.");
  },
...
const fs = {
  ...
  futimesSync() {},
  watch() {
    throw new Error("fs.watch is not supported in Onecast extensions.");
  },
...
const childProcess = {
  ...
  fork() {
    throw new Error("child_process.fork is not supported in Onecast extensions.");
  },
};
...
const zlibImpl = {
  ...
  brotliCompressSync: () => {
    throw new Error("zlib brotli is not supported in Onecast extensions.");
  },
...
const util = {
  ...
  /// modules Onecast only stubs, and a throw there would take down an extension that never reaches
  /// the code path.
...
function makeUnsupported(label) {
  const reason = `${label} is not supported in Onecast extensions (no Node runtime). See docs/extensions.md.`;
...
function requireStub(name) {
  throw new Error(`createRequire is not supported in Onecast extensions (tried to load "${name}").`);
}
