// The single seam between the JS runtime and Swift. Swift installs `__onecastHost` on the global
// before evaluating the bundle; everything else in here goes through these helpers.

const raw = globalThis.__onecastHost;
if (!raw) throw new Error("__onecastHost missing — the runtime was evaluated outside Onecast.");
...
/// `__onecast.settle`, so the JS thread never blocks waiting on the main actor.
