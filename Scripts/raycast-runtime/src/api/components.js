  ...
  return function Field(props) {
    ...
    return h(
      type,
      {
        ...rest,
        value,
        onOnecastChange: (next) => {
          const decoded = type === "Form.DatePicker" ? decodeDate(next) : next;
          setValue(decoded);
          props.onChange?.(decoded);
        },
        onOnecastBlur: props.onBlur ? () => props.onBlur({ target: { value } }) : undefined,
        onOnecastFocus: props.onFocus ? () => props.onFocus({ target: { value } }) : undefined,
      },
      props.children,
    );
  };
...
Action.PickDate = function ActionPickDate(props) {
  return h(Action, {
    ...
    onOnecastChange: (value) => props.onChange?.(decodeDate(value)),
  });
};
