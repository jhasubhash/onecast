// Entry point. Installs the polyfills and the module registry, then exposes `__onecast` — the only
...
defineModule("react-dom", {
  render: () => {
    throw new Error("react-dom is not available — Onecast renders extensions natively.");
  },
...
const hostCalls = {
  render: (sessionId, json) => globalThis.__onecastHost.render(sessionId, json),
  failed: (sessionId, message) => globalThis.__onecastHost.failed(sessionId, message),
  navigationDepthChanged: (sessionId, depth) =>
    globalThis.__onecastHost.navigationDepthChanged(sessionId, String(depth)),
  finished: (sessionId) => globalThis.__onecastHost.finished(sessionId),
};
...
setFieldCommandHandler((command, fieldId) => {
  globalThis.__onecastHost.fieldCommand(String(command), String(fieldId ?? ""));
});

globalThis.__onecast = {
