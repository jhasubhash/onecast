...
struct MenuSearchTests {
    ...
    static func targetClassify() {
        ...
        expect(
            classify("Onecast", isSelf: true, hasMenuBar: false) == .selfTarget,
            "self wins over the menu-bar check, since Onecast itself runs accessory")
        expect(
        ...
        expect(
            classify("Onecast", isSelf: true, isExcluded: true) == .selfTarget,
            "self still wins: Onecast has no menu to exclude in the first place")
    }
