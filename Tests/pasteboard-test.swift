...
// reader's own running Onecast as a genuine copy.
import AppKit
...
struct PasteboardTests {
    ...
    static func everyFileFlavourIsRead() {
        withScratch { dir in
            let file = dir.appendingPathComponent("Onecast-Settings-2026-08-20.json")
            ...
            text.setString("Onecast-Settings-2026-08-20.json", forType: .string)
            expect(
    ...
    static func volatileAndMissingFilesFallThrough() {
        let temp = URL(fileURLWithPath: "/private/tmp/onecast-volatile-\(UUID().uuidString).png")
    ...
    static func withScratch(_ body: (URL) -> Void) {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("onecast-pasteboard-test-\(UUID().uuidString)", isDirectory: true)
