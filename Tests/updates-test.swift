...
struct UpdatesTests {
    ...
    static func derivesChannels() {
        let stable = ReleaseChannel(bundleID: "com.onecast.app")
        let beta = ReleaseChannel(bundleID: "com.onecast.app.beta")
        let dev = ReleaseChannel(bundleID: "com.onecast.app.dev")
    ...
    static func entry(
        tag: String, prerelease: Bool, draft: Bool = false, assets: [String] = ["Onecast-x.zip"],
        body: String = "Notes."
    ) -> String {
        let list = assets.map {
            """
    ...
    static func rejectsUnusableFeeds() {
        ...
        expect(
            ReleaseFeed.newest(
                from: feed(entry(tag: "v0.3.0", prerelease: false, assets: ["Onecast-x.dmg"])),
    ...
    static func picksTheZipThisMacCanRun() {
        let both = feed(
            entry(
                ...
                assets: ["Onecast-0.3.0.zip", "Onecast-Universal-0.3.0.zip"]))
        expect(
        ...
        let thinOnly = feed(entry(tag: "v0.3.0", prerelease: false, assets: ["Onecast-0.3.0.zip"]))
        expect(
        ...
        let universalOnly = feed(
            entry(tag: "v0.3.0", prerelease: false, assets: ["Onecast-Universal-0.3.0.zip"]))
        expect(
