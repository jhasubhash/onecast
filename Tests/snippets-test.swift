...
struct SnippetsTests {
    ...
    private static func testRaycastImport() {
        ...
        check(
            "Raycast import uses safe Onecast defaults",
    ...
    private static func testRepositoryStorage() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent(
            "onecast-snippets-tests-\(UUID().uuidString)",
            isDirectory: true)
        ...
        let stable = SnippetRepository(
            bundleIdentifier: "com.onecast.app",
            applicationSupportRoot: channelRoot)
        let beta = SnippetRepository(
            bundleIdentifier: "com.onecast.app.beta",
            applicationSupportRoot: channelRoot)
        let dev = SnippetRepository(
            bundleIdentifier: "com.onecast.app.dev",
            applicationSupportRoot: channelRoot)
    ...
    private static func testRepositoryConcurrency() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory.appendingPathComponent(
            "onecast-snippets-concurrency-\(UUID().uuidString)",
            isDirectory: true)
        try fm.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: root) }
        ...
        boundaryEdit.text = "Onecast edit"
        do {
    ...
    private static func testCopySelectionFallback() async {
        ...
        let backing = NSPasteboard(name: .init("onecast-copy-tests-\(UUID().uuidString)"))
    ...
    private static func testDeliveryQueueAndPasteboard() async throws {
        ...
        let backingPasteboard = NSPasteboard(
            name: .init("onecast-snippets-tests-\(UUID().uuidString)"))
        ...
        check(
            "pasteboard restoration leaves no Onecast marker on the restored clipboard",
    ...
    private static func testStoreWatcher() async throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory.appendingPathComponent(
            "onecast-snippets-watcher-\(UUID().uuidString)",
            isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let repository = SnippetRepository(
    ...
    private static func testDynamicPlaceholders() {
        ...
        // Raycast's snippet spelling resolves like Onecast's.
    ...
    private static func testKeywordPolicy() {
        ...
        check("synthetic Onecast events are classified as ignored", syntheticInput == .ignored)
    ...
    private static func testKeywordListenerLifecycle() async {
        ...
        listener.processEvent(
            ...
            secureEventInputEnabled: false)
        check(
            "real user input invalidates pending automatic delivery while Onecast events do not",
            activityCount == 1)
...
final class ClipboardManager {
    static let internalType = NSPasteboard.PasteboardType("com.onecast.internal")
    func prepareForOnecastPasteboardMutation() {}
    func synchronizeAfterOnecastPasteboardMutation(changeCount: Int) {}
}
...
enum Paster {
    static let onecastEventTag: Int64 = 0x54494E59
