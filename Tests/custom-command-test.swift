...
struct CustomCommandTests {
    ...
    static func main() async {
        ...
        setsid()
        let suiteName = "com.onecast.custom-command-tests"
        ...
        let scriptDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("onecast-scripts-\(UUID().uuidString)")
        ...
        let forwarded = await ShellCommandRunner.run(
            imported?.command ?? "", arguments: ["; touch /tmp/onecast-import-should-not-exist"],
            workingDirectory: imported?.workingDirectory)
        check(
            "an imported script receives its argument as one inert word",
            forwarded.standardOutput == "; touch /tmp/onecast-import-should-not-exist"
                && !FileManager.default.fileExists(atPath: "/tmp/onecast-import-should-not-exist"))
        ...
        let marker = await ShellCommandRunner.run("test \"$ONECAST\" = 1")
        check("the ONECAST marker is exported so a shell config can detect us", marker.succeeded)
        ...
        let injected = await collect(
            ShellCommandRunner.stream(
                "printf '%s\\n' \"$1\"", arguments: ["; touch /tmp/onecast-should-not-exist"]))
        check(
            "a value carrying shell syntax is data, not code",
            injected.log.contains("; touch /tmp/onecast-should-not-exist")
                && !FileManager.default.fileExists(atPath: "/tmp/onecast-should-not-exist"))
        ...
        let zdotdir = FileManager.default.temporaryDirectory
            .appendingPathComponent("onecast-zdotdir-\(UUID().uuidString)")
        try? FileManager.default.createDirectory(at: zdotdir, withIntermediateDirectories: true)
        try? Data("alias onecast_probe=true\n".utf8).write(
        ...
        let withEnvironment = await ShellCommandRunner.run(
            "onecast_probe", loadingShellEnvironment: true)
        check(
        ...
        let withoutEnvironment = await ShellCommandRunner.run("onecast_probe")
        check(
